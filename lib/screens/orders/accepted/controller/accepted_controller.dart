...
abstract class OrdersAcceptedController extends GetxController {
  getOrders();
  refreshOrders();
  doneDelivery(String usersid, String ordersid);
...
class OrdersAcceptedControllerImp extends OrdersAcceptedController {
  ...
  doneDelivery(String usersid, String ordersid) async {
    ...
    statusRequest = StatusRequest.loading;
    update();
    var response = await ordersAcceptedData.doneDelivery(usersid, ordersid);
    print("Full Response=========: ${response.toString()}");
    statusRequest = handlingData(response);
    if (StatusRequest.success == statusRequest) {
      if (response['status'] == "success") {
